...
int main(void) {
    ...
    do {
        ...
        if (response == 'I') {
			...
			//Everytime a song is inserted, the size increments 1
			librarySize ++;
			...
			// Sort the songs using Cocktail Sort, in alphabetical order by artist, breaking ties
    ...
    while(bottom < top && !sorted) {
        ...
        if (goingUp) {
            for(i = bottom; i < top; i++) {
				...
				//order songs in song name if they have the same artists
